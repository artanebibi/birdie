...
var draw_path = true
var nDraw = ImmediateMesh.new()
var next_move_pos = Vector3(0,0,0)

func _update_path():
	initPos = global_transform.origin
	target = targets[randf_range(0, targets.size() -1)]
	var targetPos = target.global_transform.origin
	begin = nav.get_closest_point(self.get_position())
	end = nav.get_closest_point(target.get_position())
...
func aWalking(delta):
	...
	var initialTranslation = position
#	if(path.size() > 0):
	if !path.is_empty():  # Same as above
		...
		if(distance != 0):
			next_position = position.lerp(next_position, (speed*delta)/distance)
...
#		mvandcol_teste = move_and_collide(direction * speed, true, true, true)
		...
		move_and_slide()
		var onTheWay = velocity
		if onTheWay == null:
			print("Saporra parou...")
		if(abs(position.x - next_move_pos.x) <= 0.7 and abs(position.z - next_move_pos.z) <= 0.7):
